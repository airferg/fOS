...
/**
 * Handle GitHub OAuth callback and save tokens
...
export async function GET(req: NextRequest) {
  ...
  console.log('[GitHub OAuth Callback] Received callback:', {
    hasCode: !!code,
    hasError: !!error,
    hasState: !!state,
    error,
  ...
  if (error) {
    console.error('[GitHub OAuth Callback] OAuth error:', error)
    return NextResponse.redirect(new URL(`/integrations?error=${encodeURIComponent(error)}`, req.url))
  }

  if (!code) {
    console.error('[GitHub OAuth Callback] No code parameter')
  ...
  try {
    ...
    if (!clientId || !clientSecret) {
      console.error('[GitHub OAuth Callback] Missing GitHub OAuth credentials')
      return NextResponse.redirect(new URL('/integrations?error=oauth_not_configured', req.url))
    }

    console.log('[GitHub OAuth Callback] Exchanging code for tokens...')
    ...
    if (!tokenResponse.ok) {
      ...
      console.error('[GitHub OAuth Callback] Token exchange failed:', errorData)
      return NextResponse.redirect(new URL('/integrations?error=token_exchange_failed', req.url))
    }

    const tokenData = await tokenResponse.json()
    console.log('[GitHub OAuth Callback] Tokens received successfully')

    // GitHub tokens don't expire, but we'll store the scope info
    ...
    if (insertError) {
      console.error('[GitHub OAuth Callback] Error storing tokens in oauth_tokens:', insertError)
    ...
    await supabaseAdmin
      ...
      .then(({ error }) => {
        if (error) {
          console.error('[GitHub OAuth Callback] Error storing token in integration_tokens:', error)
        } else {
          console.log('[GitHub OAuth Callback] GitHub token stored successfully')
        }
      })
