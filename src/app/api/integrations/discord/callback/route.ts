...
/**
 * Handle Discord OAuth callback and save tokens
...
export async function GET(req: NextRequest) {
  ...
  console.log('[Discord OAuth Callback] Received callback:', {
    hasCode: !!code,
    hasError: !!error,
    hasState: !!state,
    error,
  ...
  if (error) {
    console.error('[Discord OAuth Callback] OAuth error:', error)
    return NextResponse.redirect(new URL(`/integrations?error=${encodeURIComponent(error)}`, req.url))
  }

  if (!code) {
    console.error('[Discord OAuth Callback] No code parameter')
  ...
  try {
    ...
    if (state) {
      try {
        ...
        console.error('[Discord OAuth Callback] Invalid state parameter')
      }
    }
    ...
    if (!clientId || !clientSecret) {
      console.error('[Discord OAuth Callback] Missing Discord OAuth credentials')
    ...
    if (!tokenResponse.ok) {
      ...
      console.error('[Discord OAuth Callback] Token exchange failed:', errorData)
      return NextResponse.redirect(new URL('/integrations?error=token_exchange_failed', req.url))
    }

    const tokenData = await tokenResponse.json()
    console.log('[Discord OAuth Callback] Tokens received successfully')
    ...
    if (insertError) {
      console.error('[Discord OAuth Callback] Error storing tokens in oauth_tokens:', insertError)
    ...
    await supabaseAdmin
      ...
      .then(({ error }) => {
        if (error) {
          console.error('[Discord OAuth Callback] Error storing token in integration_tokens:', error)
        } else {
          console.log('[Discord OAuth Callback] Discord token stored successfully')
        }
      })

    // Redirect to integrations page with success message
    return NextResponse.redirect(new URL('/integrations?connected=discord', req.url))
  } catch (err: any) {
