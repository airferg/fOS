...
/**
 * Handle LinkedIn OAuth callback and save tokens
...
export async function GET(req: NextRequest) {
  ...
  console.log('[LinkedIn OAuth Callback] Received callback:', {
    hasCode: !!code,
    hasError: !!error,
    hasState: !!state,
    error,
  ...
  if (error) {
    console.error('[LinkedIn OAuth Callback] OAuth error:', error)
    return NextResponse.redirect(new URL(`/integrations?error=${encodeURIComponent(error)}`, req.url))
  }

  if (!code) {
    console.error('[LinkedIn OAuth Callback] No code parameter')
  ...
  try {
    ...
    if (state) {
      try {
        ...
        console.error('[LinkedIn OAuth Callback] Invalid state parameter')
      }
    }
    ...
    if (!clientId || !clientSecret) {
      console.error('[LinkedIn OAuth Callback] Missing LinkedIn OAuth credentials')
    ...
    console.log('[LinkedIn OAuth Callback] Tokens received successfully')
      ...
      .upsert(
        {
          ...
          refresh_token: null, // LinkedIn doesn't provide refresh tokens in standard OAuth
    ...
    if (insertError) {
      console.error('[LinkedIn OAuth Callback] Error storing tokens in oauth_tokens:', insertError)
    ...
    await supabaseAdmin
      ...
      .then(({ error }) => {
        if (error) {
          console.error('[LinkedIn OAuth Callback] Error storing token in integration_tokens:', error)
        } else {
          console.log('[LinkedIn OAuth Callback] LinkedIn token stored successfully')
        }
      })
    ...
    console.error('[LinkedIn OAuth Callback] Unexpected error:', err)
