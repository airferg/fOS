...
/**
 * Handle Twitter/X OAuth callback and save tokens
...
export async function GET(req: NextRequest) {
  ...
  console.log('[Twitter OAuth Callback] Received callback:', {
    hasCode: !!code,
    hasError: !!error,
    hasState: !!state,
    error,
  ...
  if (error) {
    console.error('[Twitter OAuth Callback] OAuth error:', error)
    return NextResponse.redirect(new URL(`/integrations?error=${encodeURIComponent(error)}`, req.url))
  }

  if (!code) {
    console.error('[Twitter OAuth Callback] No code parameter')
  ...
  try {
    ...
    if (state) {
      try {
        ...
        console.error('[Twitter OAuth Callback] Invalid state parameter')
      }
    }
    ...
    if (!clientId || !clientSecret) {
      console.error('[Twitter OAuth Callback] Missing Twitter OAuth credentials')
      return NextResponse.redirect(new URL('/integrations?error=oauth_not_configured', req.url))
    }

    if (!codeVerifier) {
      console.error('[Twitter OAuth Callback] Missing code verifier for PKCE')
    ...
    if (!tokenResponse.ok) {
      ...
      console.error('[Twitter OAuth Callback] Token exchange failed:', errorData)
      return NextResponse.redirect(new URL('/integrations?error=token_exchange_failed', req.url))
    }

    const tokenData = await tokenResponse.json()
    console.log('[Twitter OAuth Callback] Tokens received successfully')
    ...
    if (insertError) {
      console.error('[Twitter OAuth Callback] Error storing tokens in oauth_tokens:', insertError)
    ...
    await supabaseAdmin
      ...
      .then(({ error }) => {
        if (error) {
          console.error('[Twitter OAuth Callback] Error storing token in integration_tokens:', error)
        } else {
          console.log('[Twitter OAuth Callback] Twitter token stored successfully')
        }
      })

    // Redirect to integrations page with success message
    return NextResponse.redirect(new URL('/integrations?connected=twitter', req.url))
  } catch (err: any) {
