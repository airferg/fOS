...
/**
 * Handle Notion OAuth callback and save tokens
...
export async function GET(req: NextRequest) {
  ...
  console.log('[Notion OAuth Callback] Received callback:', {
    hasCode: !!code,
    hasError: !!error,
    hasState: !!state,
    error,
  ...
  if (!code) {
    console.error('[Notion OAuth Callback] No code parameter')
  ...
  try {
    ...
    if (state) {
      try {
        ...
        console.error('[Notion OAuth Callback] Invalid state parameter')
      }
    }
    ...
    if (!clientId || !clientSecret) {
      console.error('[Notion OAuth Callback] Missing Notion OAuth credentials')
    ...
    if (!tokenResponse.ok) {
      ...
      console.error('[Notion OAuth Callback] Token exchange failed:', errorData)
    ...
    // Notion tokens don't expire, but we'll store the workspace info
    ...
    if (insertError) {
      console.error('[Notion OAuth Callback] Error storing tokens in oauth_tokens:', insertError)
    ...
    await supabaseAdmin
      ...
      .then(({ error }) => {
        if (error) {
          console.error('[Notion OAuth Callback] Error storing token in integration_tokens:', error)
        } else {
          console.log('[Notion OAuth Callback] Notion token stored successfully')
        }
      })

    // Redirect to integrations page with success message
    return NextResponse.redirect(new URL('/integrations?connected=notion', req.url))
  } catch (err: any) {
    console.error('[Notion OAuth Callback] Unexpected error:', err)
